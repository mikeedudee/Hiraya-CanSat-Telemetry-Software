...
import { Map as MapIcon, Compass, Navigation, Lock, Unlock, Layers, Signal, RotateCcw, Tag, Satellite, Home, Activity, Plus, Minus, ChevronUp, ChevronDown, Crosshair, MapPin, MessageSquare, Wind, LocateFixed, X, Cpu, Info, Move, ArrowUp, ArrowDown, Globe } from 'lucide-react';
import { calculateDistance, calculateTotalDistance, calculateSmoothedSpeed, predictLanding } from '../utils/geo';
...
export const GPSMap: React.FC<GPSMapProps> = ({ history, speedUnit, zoomSensitivity, windSpeed = 0, windDirection = 0, mapProvider, localMapPort = 8000 }) => {
  ...
  const [showCoordinates, setShowCoordinates] = useState(true); // Controls Drone Label
  
  // AETHER NOTE: Changed default to 'false' as requested
  ...
  // Map Tile Cache & Rendering Trigger
  ...
  const validHistory = useMemo(() => {
    return history.filter(p => p.latitude !== 0 || p.longitude !== 0);
  }, [history]);
  ...
  const isSignalLost = useMemo(() => {
    ...
    return (lastRaw.latitude === 0 && lastRaw.longitude === 0) && validHistory.length > 0;
  ...
  const center = useMemo(() => {
    if (isLocked && validHistory.length > 0) {
        ...
        return manualCenter;
         ...
         return { lat: validHistory[0].latitude, lon: validHistory[0].longitude };
    }
    return { lat: 0, lon: 0 };
  ...
  // Landing Prediction
  const predictedLanding = useMemo(() => {
  ...
  }, [validHistory, windSpeed, windDirection, hasFix]);


  useEffect(() => {
      if (!isLocked && hasFix && manualCenter.lat === 0 && manualCenter.lon === 0 && validHistory.length > 0) {
           const last = validHistory[validHistory.length - 1];
           setManualCenter({ lat: last.latitude, lon: last.longitude });
      }
  ...
  const handleWheel = (e: React.WheelEvent) => {
    ...
    const newZoom = Math.max(2, Math.min(19, zoom + zoomDelta));
  ...
  useEffect(() => {
    ...
    if (!hasFix && center.lat === 0 && isLocked) {
       // Wait for fix
    }
    ...
    if (validHistory.length > 1) {
        ...
        ctx.shadowBlur = 10;
        const start = toScreen(validHistory[0].latitude, validHistory[0].longitude);
        ctx.moveTo(start.x, start.y);
        let lastScreenX = start.x;
        let lastScreenY = start.y;
        for (let i = 1; i < validHistory.length; i++) {
          const pt = toScreen(validHistory[i].latitude, validHistory[i].longitude);
    ...
    // 4. Draw Predicted Landing
    if (predictedLanding) {
        const predPos = toScreen(predictedLanding.lat, predictedLanding.lon);
        ctx.save();
        ...
        ctx.textAlign = 'center';
        ctx.fillText("PREDICTED", 0, -12);
        
        ctx.restore();
    }

    // 5. Draw Drone Position & LIVE COORDINATES
    if (validHistory.length > 0) {
        ...
        if (showCoordinates) {
             ...
             ctx.strokeStyle = 'rgba(51, 65, 85, 1)'; // Slate border
             ctx.lineWidth = 1;
  ...
  }, [validHistory, zoom, center, showMapTiles, showCoordinates, showTooltips, heading, isHovering, mouseGeo, hoveringHome, canvasRef.current?.width, predictedLanding, hasFix, redrawTrigger, mapProvider, isLocked, localMapPort]);

  return (
    <div 
      ref={containerRef}
      className={`flex-1 bg-slate-900/50 border border-slate-800 rounded-xl p-0 relative flex flex-col overflow-hidden h-full w-full shadow-lg group touch-none ${isDragging ? 'cursor-grabbing' : 'cursor-crosshair'}`}
    ...
    >
      <div className="absolute top-0 left-0 right-0 p-3 flex justify-between items-start z-10 bg-gradient-to-b from-black/80 to-transparent pointer-events-none">
        <h3 className="text-slate-300 font-bold text-sm uppercase flex items-center gap-2 drop-shadow-md">
          <MapIcon className="w-4 h-4 text-cyan-400" /> GPS Track
        </h3>
      ...
      <div 
        ...
        onMouseDown={(e) => e.stopPropagation()} // Stop propagation to prevent map dragging
      >
          <button onClick={() => setZoom(z => Math.min(19, z + 1))} className="p-1.5 rounded hover:bg-slate-700 text-slate-400 hover:text-white" title="Zoom In"><Plus className="w-4 h-4"/></button>
      ...
      {/* COMPASS + WIND INDICATOR (Top Right) */}
      {hasFix && (
          <div className="absolute top-14 right-3 z-20 group cursor-help" title="Compass & Wind Direction">
              <div className="relative w-20 h-20 bg-black/40 backdrop-blur-sm rounded-full border border-slate-700 shadow-xl flex items-center justify-center hover:border-cyan-500/50 transition-colors">
                  ...
                  {/* Wind Direction Overlay (Within Compass) */}
                  {windSpeed > 0 && (
                      <div 
                        className="absolute inset-0 flex flex-col items-center justify-start pt-1.5 pointer-events-none"
                        style={{ transform: `rotate(${windDirection}deg)` }}
                      >
                          {/* Two Side-by-Side Arrows Pointing Down (With the wind flow) */}
                          <div className="flex gap-0.5 opacity-90">
                             <ArrowDown className="w-3 h-3 text-slate-300 drop-shadow-[0_0_3px_rgba(255,255,255,0.4)] animate-pulse" />
                             <ArrowDown className="w-3 h-3 text-slate-300 drop-shadow-[0_0_3px_rgba(255,255,255,0.4)] animate-pulse delay-75" />
                          </div>
                      </div>
                  )}
      ...
      {/* SENSOR FUSION INFO PANEL (Moved to Left, beside Toolbar) */}
      {hasFix && showTooltips && (
         // AETHER NOTE: Changed position to 'top-12 left-16' and animation to 'slide-in-from-left'
         <div className="absolute top-12 left-16 z-20 bg-black/80 backdrop-blur-md border border-slate-700 rounded-lg p-3 shadow-2xl animate-in fade-in slide-in-from-left-4 pointer-events-none max-w-[200px]">
             ...
             <div className="space-y-2 text-[10px] text-slate-300 leading-snug">
                 <p>
                    <span className="text-emerald-400 font-bold">POS:</span> GNSS/GPS constellation data provides global positioning.
                 </p>
                 <p>
                    <span className="text-cyan-400 font-bold">HEADING:</span> IMU (Gyro/Accel/Mag) fusion estimates orientation relative to magnetic north.
                 </p>
                 <p>
                    <span className="text-amber-400 font-bold">ALT:</span> Barometric pressure fused with GPS altitude for vertical precision.
                 </p>
             </div>
         </div>
      )}

      {/* COMPACT HOME HOVER CARD */}
      {hasFix && hoveringHome && homePosRef.current && (
          <div 
             className="absolute z-50 p-2 rounded-lg bg-slate-900/95 backdrop-blur-xl border border-emerald-500/30 shadow-[0_0_20px_rgba(16,185,129,0.15)] animate-in zoom-in-95 duration-200 pointer-events-none min-w-[120px]"
             style={{ 
                 left: homePosRef.current.x, 
                 top: homePosRef.current.y - 70, 
          ...
          >
              ...
              <div className="flex items-center gap-2 mb-1.5 pb-1 border-b border-slate-800">
                  ...
                  <div>
                      <h4 className="text-[10px] font-bold text-white leading-none">Home</h4>
                      <span className="text-[8px] text-emerald-500 font-mono uppercase leading-none">Origin</span>
                  </div>
              </div>
              ...
              <div className="space-y-0.5 text-[9px] font-mono">
                  <div className="flex justify-between gap-2">
                      ...
                      <span className="text-emerald-100">{homePosRef.current.lat.toFixed(6)}</span>
                  </div>
                  <div className="flex justify-between gap-2">
                      <span className="text-slate-500">LON</span>
                      <span className="text-emerald-100">{homePosRef.current.lon.toFixed(6)}</span>
                  </div>
              </div>
          </div>
      )}
      ...
      <div className="absolute bottom-3 right-3 flex flex-col items-end gap-2 z-20 pointer-events-none">
           ...
           {/* LEGEND + WIND STATS ROW */}
           {hasFix && (
               <div className="pointer-events-auto flex items-end gap-2 mb-1">
                   {/* Wind Stats Box */}
                   {windSpeed > 0 && (
                       <div className="bg-black/60 backdrop-blur border border-slate-700 rounded-lg px-2 py-1.5 shadow-xl flex items-center gap-2" title="Wind Conditions (Speed & Direction From)">
                           <Wind className="w-3 h-3 text-cyan-400" />
                           <div className="flex flex-col leading-none">
                               <span className="text-[9px] font-bold text-white">{windSpeed} m/s</span>
                               <span className="text-[8px] font-mono text-slate-400">@{windDirection}°</span>
                           </div>
                       </div>
                   )}
                   ...
                   <div className="flex flex-col items-end">
                        {isLegendOpen && (
                            <div className="bg-black/80 backdrop-blur border border-slate-700 rounded-t-lg p-3 w-32 shadow-xl text-[9px] text-slate-300 space-y-2 animate-in slide-in-from-bottom-2 mb-[-1px] z-10">
                                <div className="flex items-center justify-between text-[8px] font-bold text-slate-500 uppercase border-b border-slate-800 pb-1 mb-2">
                                    Map Legend
                                </div>
                                {/* AETHER NOTE: Updated Legend to match new features */}
                                <div className="flex items-center gap-2">
                                    <Tag className="w-3 h-3 text-cyan-400" />
                                    <span>Live Telemetry</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className={`w-4 h-0.5 rounded shadow-sm ${showMapTiles ? 'bg-rose-500 shadow-rose-500/50' : 'bg-emerald-500 shadow-emerald-500/50'}`}></div>
                                    <span>Flight Path</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="w-0 h-0 border-l-[4px] border-l-transparent border-r-[4px] border-r-transparent border-b-[8px] border-b-white drop-shadow-md"></div>
                                    <span>Drone Pos</span>
                                </div>
                                <div className="flex items-center gap-2">
                                ...
                                {predictedLanding && (
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-rose-400">Pred. Landing</span>
                                    </div>
                                )}
                            </div>
                        )}
                        <button 
                            ...
                            className={`flex items-center gap-2 bg-black/60 backdrop-blur border border-slate-700 px-3 py-1.5 ${isLegendOpen ? 'rounded-b-lg border-t-0' : 'rounded-lg'} text-[10px] font-bold text-slate-400 hover:text-white transition-all hover:bg-slate-800 shadow-xl`}
                            title="Toggle Map Legend Visibility"
                        >
           ...
           {/* CONTAINER FOR CURSOR TOOLTIP + STATS */}
           <div className="flex items-end gap-2">
               {/* FLOATING CURSOR DATA */}
               {isHovering && mouseGeo && (
                 <div className="bg-black/80 backdrop-blur-md border border-slate-700 rounded p-1.5 shadow-2xl pointer-events-none whitespace-nowrap">
                     <div className="flex items-center justify-between gap-2 border-b border-slate-700/50 pb-1 mb-1">
                         <div className="flex items-center gap-1">
                            <LocateFixed className="w-3 h-3 text-amber-400" />
                            <span className="text-[9px] font-bold text-slate-300">CURSOR</span>
                         </div>
                     </div>
                     <div className="space-y-0.5 text-[9px] font-mono leading-tight">
                         <div className="flex gap-3 justify-between">
                            <span className="text-slate-500">LAT</span>
                            <span className="text-white">{mouseGeo.lat.toFixed(6)}</span>
                         </div>
                         <div className="flex gap-3 justify-between">
                            <span className="text-slate-500">LON</span>
                            <span className="text-white">{mouseGeo.lon.toFixed(6)}</span>
                         </div>
                         {homePosRef.current && (
                            <div className="flex gap-3 justify-between text-emerald-400 pt-0.5 border-t border-slate-700/50 mt-0.5">
                                <span>DST</span>
                                <span>{calculateDistance(mouseGeo.lat, mouseGeo.lon, homePosRef.current.lat, homePosRef.current.lon).toFixed(0)}m</span>
                            </div>
                         )}
                     </div>
                 </div>
